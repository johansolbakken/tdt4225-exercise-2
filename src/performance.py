...
def print_timer(timer):
    ...
    if duration.total_seconds() < 1:
        ...
        log.info("{} took {:.2f}ms".format(timer.name,duration))
    elif duration.total_seconds() < 60:
        duration = duration.total_seconds()
        log.info("{} took {:.2f}s".format(timer.name,duration))
    elif duration.total_seconds() < 60 * 60:
        duration = duration.total_seconds() / 60
        log.info("{} took {:.2f}m".format(timer.name,duration))
    else:
        duration = duration.total_seconds() / 60 / 60
        log.info("{} took {:.2f}h".format(timer.name,duration))
